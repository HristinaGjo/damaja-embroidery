...
// Disable Lenis scroll when layout-1-gallery or layout-3-gallery is active
function toggleLenisScroll(enable) {
    if (enable) {
        lenis.start();  // Enable smooth scrolling
    } else {
...
function switchLayoutHandler(newLayout) {
    ...
    if (newLayout === "layout-2-gallery") {
        ...
        toggleLenisScroll(true); // Enable scrolling for layout-2-gallery
    } else {
        gsap.to([imgPreviews, minimap], {
            autoAlpha: 0,
            duration: 0.3,
        ...
        // Disable scroll for layout-1-gallery and layout-3-gallery
        toggleLenisScroll(!(newLayout === "layout-1-gallery" || newLayout === "layout-3-gallery"));
    }
